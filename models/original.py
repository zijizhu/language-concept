...
class OursNet(nn.Module):
    def __init__(self, features, img_size, prototype_shape,
        ...
        self.features = nn.Sequential(*list(densenet161(weights=DenseNet161_Weights.DEFAULT).children())[:-1])

        features_name = "densenet161".upper()
        if features_name.startswith('VGG'):
            ...
            first_add_on_layer_in_channels = 2208
    ...
    def get_SDFA_loss(self, proto_acts, shallow_feas, deep_feas, label, consis_thresh=None):
        ...
        proto_indices += torch.arange(proto_per_class).to(
            device=device)  # (B, 10), get 10 indices of activation maps of each sample
    ...
    def forward(self, x):
        ...
        # shallow_feas = shallow_feas.reshape(batch_size, dim, fea_size, shallow_size // fea_size, fea_size,
        #                                     shallow_size // fea_size)
        ...
        return logits, (cosine_min_distances, project_activations, None, None)
    ...
    def forward_SA_module(self, prototype_activations):
        ...
        logits = prototype_activations.reshape(-1, self.num_classes, self.num_prototypes_per_class).sum(
            dim=-1)  # (B, 200)
...
def construct_OursNet(base_architecture, pretrained=True, img_size=224,
    ...
    return OursNet(features=base_architecture,
                   img_size=img_size,
